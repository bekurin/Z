...
written. The gate opens only when `ambiguity <= 0.2`.

## Constants

```
WEIGHTS (greenfield) : goal 0.40 | constraint 0.30 | success 0.30
WEIGHTS (brownfield) : goal 0.34 | constraint 0.26 | success 0.26 | context 0.14   (renormalized)
THRESHOLD            : 0.20
MAX_ROUNDS           : 8
...
- `ambiguity`: final `score`, `threshold` 0.2, `forced` boolean, and per-dimension
  `{weight, clarity}`.
- `created_at`: current UTC ISO-8601. `frozen`: `true`.
- `content_hash`: `"sha256:" + sha256(` the card JSON with `content_hash` set to `""`,
  keys sorted, compact separators `)`. Compute it however is available — e.g.:
  ```bash
  # after writing the card WITHOUT content_hash (or with it blank):
  python3 - "$CARD_PATH" <<'PY'
  import hashlib, json, sys
  p = sys.argv[1]
  card = json.load(open(p))
  card["content_hash"] = ""
  h = hashlib.sha256(json.dumps(card, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
  card["content_hash"] = "sha256:" + h
  json.dump(card, open(p, "w"), indent=2)
  PY
  ```
  If no shell is available, still fill every other field and note the hash was not computed.

Write the file to the **project** (not the plugin dir) at
`.z/spec-cards/<id>.json` using the Write tool. Create `.z/spec-cards/` if needed. Cards
are append-only — never overwrite an existing card; a change means a new card with
`parent_id` set.
